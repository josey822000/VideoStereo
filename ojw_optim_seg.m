function [D info] = ojw_optim_seg(vals, plane, options, Dinit, SegMap,pid)
...
sz = size(SegMap);
d_min = vals.d_min;
d_max = d_min + vals.d_step;
%max_iters = options.max_iters + options.average_over + 1;
max_iters = segNum + options.max_iters + 1;
...
D = Dinit;
info.energy(1:segNum,end) = realmax;
info.energy(iter,end) = realmax / 1e20;
while (1-(info.energy(iter,end)/info.energy(iter-segNum,end)) > options.converge) && iter < max_iters
	iter = iter + 1;
	sid = mod(iter-(segNum+1), segNum)+1;
    % Set the new (proposal) depth map
    t_start = cputime; % Start timing
	
	[X Y] = meshgrid(sz(2),sz(1));
	Dnew = -(X * plane{pid}.N(sid,1) + Y * plane{pid}.N(sid,2) + plane{pid}.N(sid,3));
	out_bound = Dnew<d_min || Dnew>d_max;
	Dnew(out_bound) = D(out_bound);
	Dnew = double(Dnew);
...
sortedSeg = unique(SegMap);
for s=1:numel(sortedSeg)
	SegMap(SegMap==sortedSeg(s))=s;
end
