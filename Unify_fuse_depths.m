function [N info energy V] = Unify_fuse_depths(D1, D2, vals, D1_Obj, D2_Obj, options)


% $Id: ibr_fuse_depths.m,v 1.3 2008/11/17 11:27:35 ojw Exp $
t_start = cputime;
figure(2);imshow(D1/vals.disps(1));
...
Rf = P.R(:,:,1);
Tf = P.T(:,1);
CountDifferentLabel = zeros(tp,2, class(Kinf));
for a = 1:num_in
    ...
    % Epc
    % D1,D2
    % Here the otherview should directly use other key frame
...
%     imshow(Check2);
    ...
    CheckWarpErr = [reshape(D1_Obj.segMap~=0,[],1); reshape(D2_Obj.segMap~=0,[],1)];
    occ = occ & CheckDisRange & CheckWarpErr;
    Check = Check & CheckDisRange & CheckWarpErr;
    ...
    % Obj Color term
    Ecol = reshape(lambdaColor*-GMM(vals.R,D1_Obj.segMap,D2_Obj.segMap,D1_Obj.GMM_Name,D2_Obj.GMM_Name,D1_Obj.table,D2_Obj.table,D1_Obj.Key,D2_Obj.Key),tp,2);
    
    % Obj parallax
    Epar = reshape(lambdaParallax*(1-ParallaxModel(D1,D2,D1_Obj,D2_Obj,vals.d_step,vals.ndisps,D1_Obj.table,D2_Obj.table)),tp,2);
...
function outval = ParallaxModel(D1,D2,ObjModel1,ObjModel2,d_step,ndisps,D1_table,D2_table)
    ...
    imshow(depth1/0.0087);
    figure(15); imshow(reshape(probMap2,size(ObjModel1.segMap)));
    imshow(depth2/0.0087);
    outval = [reshape(probMap1,[],1) ; reshape(probMap2,[],1)];
return


function [outval] = GMM(R,ObjMap1,ObjMap2,ModelName1,ModelName2,D1_table,D2_table,D1_Key,D2_Key)
    halfR = R(1:end/2,:);
    probMap1 = zeros(size(halfR,1),1);
    probMap2 = zeros(size(halfR,1),1);
    ObjIds = unique(ObjMap1);
    if ObjIds(1) ==0
        probMap1(ObjMap1==0) = -100;
        ObjIds = ObjIds(2:end);
    end
    for i= 1:numel(ObjIds)
        s = ObjIds(i);
        samples = halfR(ObjMap1==s,:);
        prob = PredictGMM(samples',fullfile(D1_Key,ModelName1{D1_table(s)}));
        probMap1(ObjMap1==s) = prob;
    end
    ObjIds = unique(ObjMap2);
    if ObjIds(1) ==0
        probMap2(ObjMap2==0) = -100;
        ObjIds = ObjIds(2:end);
    end
    for i= 1:numel(ObjIds)
        s = ObjIds(i);
        samples = halfR(ObjMap2==s,:);
        prob = PredictGMM(samples',fullfile(D2_Key,ModelName2{D2_table(s)}));
        probMap2(ObjMap2==s) = prob;
    end
    probMap1(probMap1<-100) = -100;
    probMap2(probMap2<-100) = -100;
    outval = [reshape(probMap1,[],1) ; reshape(probMap2,[],1)];
