function [D info ObjModel] = obj_fuse_proposals(vals, Dproposals, options, Model, Expan)
...
%     [M stats info.energy(iter,:) V] = obj_fuse_depths(D, Dnew, vals, ObjModel,Model.segpln_Obj{n},options);
        if Expan
            [M stats info.energy(iter,:) V] = Expan_fuse_depths(D, Dnew, vals, ObjModel,Model{n},options);
        else 
            [M stats info.energy(iter,:) V] = obj_fuse_depths(D, Dnew, vals, ObjModel,Model{n},options);
        end
    try
        a = 1;
    catch
    ...
    if Expan
        [D ObjModel] = ExpanRefreshObj(M,D,Dnew,ObjModel,Model{n},R,iter,vals.disps,vals.SEI,permute(vals.P,[2 1 3]));
    else 
